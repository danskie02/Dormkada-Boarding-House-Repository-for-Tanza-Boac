...
2. **Authentication API** (`artifacts/api-server/src/routes/auth.ts`)
   - Modified `POST /auth/register` endpoint
...
2. **Boarding Houses API** (`artifacts/api-server/src/routes/boarding-houses.ts`)
   - Modified `POST /boarding-houses` endpoint
...
### Modified Endpoints:
- `POST /api/reservations` - Now sends owner notification
- `POST /auth/register` - Now sends admin notification for owner registrations
- `POST /api/boarding-houses` - Now sends admin notification for new listings
