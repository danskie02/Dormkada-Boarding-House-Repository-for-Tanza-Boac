...
router.post("/reservations/:id/cancel", requireAuth, requireRole("student"), async (req, res): Promise<void> => {
  ...
  const txResult = await db.transaction(async (tx) => {
    const [reservation] = await tx.select().from(reservationsTable).where(eq(reservationsTable.id, id));
    if (!reservation) return { kind: "not_found" as const };
    if (reservation.status !== "accepted") return { kind: "bad_status" as const };
    if (reservation.studentId !== req.user!.userId) return { kind: "forbidden" as const };
    ...
    const [room] = await tx.select().from(roomsTable).where(eq(roomsTable.id, reservation.roomId));
    if (room) {
      const newAvailable = room.availableSlots + 1;
      const newStatus = newAvailable > 1 ? "available" : newAvailable === 1 ? "almost_full" : "full";
      await tx.update(roomsTable).set({ availableSlots: newAvailable, status: newStatus }).where(eq(roomsTable.id, room.id));
      ...
      const allRooms = await tx.select().from(roomsTable).where(eq(roomsTable.boardingHouseId, room.boardingHouseId));
      const availableRooms = allRooms.filter((r) => r.status !== "full").length;
      await tx.update(boardingHousesTable).set({ availableRooms }).where(eq(boardingHousesTable.id, room.boardingHouseId));
    }

    return { kind: "ok" as const, updated: updated!, reservation };
  });
  ...
  const { updated, reservation } = txResult;
